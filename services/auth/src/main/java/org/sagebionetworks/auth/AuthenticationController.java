...
import org.sagebionetworks.authutil.ReadFileUtil;
	...
	public static boolean acceptsTermsOfUse(String userId, Boolean acceptsTermsOfUse) throws NotFoundException, IOException {
		...
		if (!getAcceptsTermsOfUse(userId)) {
			if (acceptsTermsOfUse!=null && acceptsTermsOfUse==true) {
				...
				//throw new ForbiddenException(TERMS_OF_USE);
				return false;
			}
		}	
		return true;
...
//	private static final String TERMS_OF_USE = ReadFileUtil.readFileContentFromClasspath(AuthenticationController.class, "termsOfUse");
		...
		try { 
			...
			if (session==null) { // not using cache or not found in cache
				...
				if (!acceptsTermsOfUse(credentials.getEmail(), credentials.isAcceptsTermsOfUse()))
					throw new ForbiddenException("You need to sign the Synapse Terms of Use.");
				if (cacheTimeout>0) {
	...
	void openIDCallback(
		...
		try {
			...
			if (acceptsTermsOfUse(email, acceptsTermsOfUse)) {
				...
				redirectUrl += AuthorizationConstants.ACCEPTS_TERMS_OF_USE_REQUIRED_TOKEN;
			}
