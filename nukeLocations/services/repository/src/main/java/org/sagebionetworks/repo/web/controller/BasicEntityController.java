...
/**
 * Provides generic access to all entities. The caller does not need to know the
 * entity type for these methods. New entity types should automatically be
 * exposed with these methods.
 * 
 * @author John
 * 
 */
...
public class BasicEntityController extends BaseController {
	...
	Annotations updateEntityAnnotations(
		...
		// Pass it along
		return entityController.updateEntityAnnotations(userId, id,
	...
	Entity createEntity(
		...
		Entity createdEntity = entityController.createEntity(userId, entity,
				request);
	...
	Entity updateEntity(
			@RequestParam(value = AuthorizationConstants.USER_ID_PARAM, required = false) String userId,
			@RequestHeader HttpHeaders header, @PathVariable String id,
			@RequestHeader(ServiceConstants.ETAG_HEADER) String etag,
			HttpServletRequest request) throws NotFoundException,
			ConflictingUpdateException, DatastoreException,
			InvalidModelException, UnauthorizedException, IOException,
			JSONObjectAdapterException {
		// Note that we auto-version for locationable entities whose md5
	...
	Entity getEntityForVersion(
		...
		Entity updatedEntity = entityController.getEntityForVersion(userId, id,
				versionNumber, request, type.getClassForType());
		// Return the results
		return updatedEntity;
	}

	/**
	 * Get the EntityHeader for an existing entity with a GET.
	 * 
	 * @param userId
	 *            -The user that is doing the get.
	 * @param id
	...
	PaginatedResults<EntityHeader> getEntityReferences(
		...
		// Get the type of an entity by ID.
		return entityController.getEntityReferences(userId, id, null, offset,
				limit, request);
	}
	...
	PaginatedResults<EntityHeader> getEntityReferences(
			...
			@RequestParam(value = ServiceConstants.PAGINATION_OFFSET_PARAM, required = false, defaultValue = ServiceConstants.DEFAULT_PAGINATION_OFFSET_PARAM) Integer offset,
			@RequestParam(value = ServiceConstants.PAGINATION_LIMIT_PARAM, required = false, defaultValue = ServiceConstants.DEFAULT_PAGINATION_LIMIT_PARAM) Integer limit,
			@PathVariable String id, @PathVariable int versionNumber,
			HttpServletRequest request) throws NotFoundException,
			DatastoreException {
		// Get the type of an entity by ID.
